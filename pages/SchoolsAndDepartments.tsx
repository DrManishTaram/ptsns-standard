          ...
          <div className="flex flex-col md:flex-row gap-4 justify-center items-center max-w-5xl mx-auto">
            ...
            <div className="relative w-full md:w-1/2">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 rounded-full border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-base bg-white shadow-sm hover:shadow-md transition-all placeholder:text-gray-400"
              />
            </div>
            ...
            <div className="relative w-full md:w-1/2">
              ...
              <select
                ...
                className="w-full pl-6 pr-10 py-3 rounded-full border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-base bg-white shadow-sm hover:shadow-md transition-all appearance-none cursor-pointer text-gray-700 font-medium"
              >
                {tabItems.map(tab => (
      ...
      <section className="py-12 bg-slate-50 min-h-[60vh]">
        <div className="container mx-auto px-4">
          ...
          {displaySchools.length === 0 ? (
            ...
            <>
              {activeTab === 'all' ? (
                /* ACCORDION VIEW FOR ALL SCHOOLS */
                <div className="space-y-4 max-w-5xl mx-auto">
                  {displaySchools.map(school => (
                    <div key={school.id} className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden transition-all hover:shadow-md">
                      {/* Accordion Header */}
                      <button
                        onClick={() => toggleSchool(school.id)}
                        className="w-full px-6 py-5 flex items-center justify-between text-left bg-white hover:bg-slate-50 transition-colors group"
                      >
                        <div className="pr-4">
                          <h2 className="text-xl font-bold text-[#071133] group-hover:text-blue-800 transition-colors">{school.title}</h2>
                          <p className="text-slate-500 text-sm mt-1 line-clamp-1">{school.description}</p>
                        </div>
                        <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${openSchools[school.id] ? 'bg-blue-100 rotate-180' : 'bg-slate-50 group-hover:bg-blue-50'}`}>
                          {openSchools[school.id] ? (
                            <Minus className="text-blue-600" strokeWidth={3} size={20} />
                          ) : (
                            <Plus className="text-blue-600" strokeWidth={3} size={20} />
                          )}
                        </div>
                      </button>
                      ...
                      {/* Accordion Body */}
                      <div
                        className={`transition-all duration-300 ease-in-out origin-top ${openSchools[school.id] ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'
                          }`}
                      >
                        <div className="p-6 pt-2 border-t border-slate-100 bg-slate-50/30">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {school.departments.map(dept => (
                              <Link to={dept.link} key={dept.id} className="group/card flex items-center justify-between p-4 bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md hover:border-blue-200 transition-all border-l-[4px] border-l-blue-600">
                                <div className="flex items-center">
                                  <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-blue-50 flex items-center justify-center mr-3 group-hover/card:bg-blue-600 transition-colors">
                                    <dept.icon size={18} className="text-blue-600 group-hover/card:text-white transition-colors" />
                                  </div>
                                  <span className="font-bold text-slate-700 text-base group-hover/card:text-blue-700 transition-colors leading-tight">
                                    {dept.name}
                                  </span>
                                </div>
                                <div className="bg-green-50 p-1.5 rounded-full group-hover/card:bg-green-100 transition-colors">
                ...
                /* DETAILED GRID VIEW FOR SINGLE SCHOOL */
                displaySchools.map(school => (
